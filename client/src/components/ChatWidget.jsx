...
import React, { useState } from "react";
import botAvatar from "/assets/bot-avatar.jpeg";   // Bot avatar
import userAvatar from "/assets/user-avatar.jpeg"; // User avatar (can be your logo or any icon)
...
export default function ChatWidget() {
  ...
  return (
    <div className="chat">
      {!open && (
        <button className="chat-button" onClick={() => setOpen(true)}>
  <img src={botAvatar} alt="Zyra" className="chat-button-avatar" />
  <span className="chat-button-text">Zyra</span>
</button>
      )}
      {open && (
        <div className="chat-panel">
          <div className="chat-header">
            <img src={botAvatar} alt="Bot Avatar" className="bot-logo" />
            <strong>Zyra</strong>
            <button onClick={() => setOpen(false)} aria-label="close">
              ✕
            </button>
          ...
          <div className="chat-body">
            {messages.map((m, i) => (
              <div key={i} className={`msg-row ${m.from}`}>
                <img
                  src={m.from === "bot" ? botAvatar : userAvatar}
                  alt={m.from}
                  className="msg-avatar"
                />
                <p className={m.from === "bot" ? "bot-msg" : "user-msg"}>
                  {m.text}
                </p>
              </div>
            ))}
